...
пользователям, кроме UserCreateApiView и SupplierRetrieveApiView (чтобы можно по ссылке в админе просмотреть данные поставщика)
пример:
![img.png](img.png)
10. для запуска теста: coverage run manage.py test 
1посмотреть покрытие тестами: coverage report  
...
- **title**: текстовое поле длиной не более 150 символов (подробное название: "название").
- **model**: Текстовое поле, которое должно быть уникальным и иметь максимальную длину 150 символов (подробное название: "модель").
...
- **title**: текстовое поле длиной не более 150 символов (подробное название: "название").
- **level**: поле символов с несколькими вариантами выбора, представляющими такие уровни, как "завод", "родственная сеть", "индивидуальный предприниматель".
- **email**: поле электронной почты, которое должно быть уникальным (подробное название: "электронная почта").
- **country, city, street, house_number**: текстовые поля, в которых хранятся данные адреса.
- **products**: связь "Многие ко многим" с моделью продукта (подробное название: "продукты").
...
- **email**: поле электронной почты, которое должно быть уникальным (подробное название: "электронная почта").
- **country, city, street, house_number**: Текстовые поля, в которых хранятся сведения об адресе.

### Сеть

- **name**: поле для ввода символов длиной не более 150 символов, которое должно быть уникальным (подробное имя: "название").
- **supplier**: Связь внешнего ключа с моделью поставщика (при удалении: КАСКАДНАЯ).
- **supplier_debt**: Десятичное поле для хранения суммы долга (подробное название: "задолженность перед поставщиками").
- **contact**: связь внешнего ключа с моделью контакта (при удалении: КАСКАД).
...
Кроме того, поле level доступно только для чтения и включено в сериализацию.
...
- /product/list/: вывод списка продуктов (ProductListApiView)
...
- /supplier/retrieve/<int:pk>/: Извлечение поставщика по его идентификатору (SupplierRetrieveApiView)
- /supplier/destroy/<int:pk>/: Уничтожение или удаление поставщика по его идентификатору (SupplierDestroyApiView)
- /supplier/list/: Список всех поставщиков (SupplierListApiView)

## URL-адреса сети
- /network/create/: Создать новую сеть (NetworkCreateApiView)
- /network/update/<int:pk>/: Обновить сеть по ее идентификатору (NetworkUpdateApiView)
- /network/retrieve/<int:pk>/: Извлекает сеть по ее идентификатору (NetworkRetrieveApiView)
- /network/destroy/<int:pk>/: Уничтожить или удалить сеть по ее идентификатору (NetworkDestroyApiView)
- /network/list/: Список всех сетей (NetworkListApiView)

## Адреса контактов
- /contact/create/: Создать новый контакт (ContactCreateApiView)
- /contact/update/<int:pk>/: Обновить контакт по его идентификатору (ContactUpdateApiView)
- /contact/retrieve/<int:pk>/: Получить контакт по его идентификатору (ContactRetrieveApiView)
